...
// New creates a new Koboldcpp provider with the given API key.
func New(apiKey string, opts ...Option) (core.Provider, error) {
	...
	if p.client.BaseURL == "" {
		return nil, fmt.Errorf("koboldcpp: base URL is required, use WithBaseURL")
	}
	return p, nil
}

// Option configures the Koboldcpp provider.
...
// Models returns a list of available models from the Koboldcpp API.
...
// LanguageModel creates a new Koboldcpp language model for the given model ID.
...
// EmbeddingModel creates a new Koboldcpp embedding model for the given model ID.
...
// ProviderOptions holds Koboldcpp-specific request options.
type ProviderOptions struct{}

// ProviderName returns the provider name for these options.
func (ProviderOptions) ProviderName() string { return "koboldcpp" }
