...
func TestProvider_LanguageModel(t *testing.T) {
	p, _ := New("sk-test")
	model, err := p.LanguageModel(context.Background(), "novita-medium")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestProvider_EmbeddingModel(t *testing.T) {
	p, _ := New("sk-test")
	prov := p.(*Provider)
	model, err := prov.EmbeddingModel(context.Background(), "novita-embed")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	em := model.(*EmbeddingModel)
	if em.Provider() != "novita" {
		t.Errorf("unexpected provider: %s", em.Provider())
	}
	if em.Model() != "novita-embed" {
