...
// New creates a new Cometapi provider with the given API key.
func New(apiKey string, opts ...Option) (core.Provider, error) {
	...
	if p.client.BaseURL == "" {
		return nil, fmt.Errorf("cometapi: base URL is required, use WithBaseURL")
	}
	return p, nil
}

// Option configures the Cometapi provider.
...
// Models returns a list of available models from the Cometapi API.
...
// LanguageModel creates a new Cometapi language model for the given model ID.
...
// EmbeddingModel creates a new Cometapi embedding model for the given model ID.
...
// ProviderOptions holds Cometapi-specific request options.
type ProviderOptions struct{}

// ProviderName returns the provider name for these options.
func (ProviderOptions) ProviderName() string { return "cometapi" }
