...
// New creates a new Foundry provider with the given API key.
func New(apiKey string, opts ...Option) (core.Provider, error) {
	...
	if p.client.BaseURL == "" {
		return nil, fmt.Errorf("foundry: base URL is required, use WithBaseURL")
	}
	return p, nil
}

// Option configures the Foundry provider.
...
// Models returns a list of available models from the Foundry API.
...
// LanguageModel creates a new Foundry language model for the given model ID.
...
// EmbeddingModel creates a new Foundry embedding model for the given model ID.
...
// ProviderOptions holds Foundry-specific request options.
type ProviderOptions struct{}

// ProviderName returns the provider name for these options.
func (ProviderOptions) ProviderName() string { return "foundry" }
