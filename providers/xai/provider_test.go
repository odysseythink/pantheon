...
func TestProvider_LanguageModel(t *testing.T) {
	p, _ := New("sk-test")
	model, err := p.LanguageModel(context.Background(), "xai-model")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestProvider_EmbeddingModel(t *testing.T) {
	p, _ := New("sk-test")
	prov := p.(*Provider)
	model, err := prov.EmbeddingModel(context.Background(), "xai-embed")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
