...
// New creates a new GenericOpenAI provider with the given API key.
func New(apiKey string, opts ...Option) (core.Provider, error) {
	...
	if p.client.BaseURL == "" {
		return nil, fmt.Errorf("genericopenai: base URL is required, use WithBaseURL")
	}
	return p, nil
}

// Option configures the GenericOpenAI provider.
...
// Models returns a list of available models from the GenericOpenAI API.
...
// LanguageModel creates a new GenericOpenAI language model for the given model ID.
...
// EmbeddingModel creates a new GenericOpenAI embedding model for the given model ID.
...
// ProviderOptions holds GenericOpenAI-specific request options.
type ProviderOptions struct{}

// ProviderName returns the provider name for these options.
func (ProviderOptions) ProviderName() string { return "genericopenai" }
