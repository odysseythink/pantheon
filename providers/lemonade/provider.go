...
// New creates a new Lemonade provider with the given API key.
func New(apiKey string, opts ...Option) (core.Provider, error) {
	...
	if p.client.BaseURL == "" {
		return nil, fmt.Errorf("lemonade: base URL is required, use WithBaseURL")
	}
	return p, nil
}

// Option configures the Lemonade provider.
...
// Models returns a list of available models from the Lemonade API.
...
// LanguageModel creates a new Lemonade language model for the given model ID.
...
// EmbeddingModel creates a new Lemonade embedding model for the given model ID.
...
// ProviderOptions holds Lemonade-specific request options.
type ProviderOptions struct{}

// ProviderName returns the provider name for these options.
func (ProviderOptions) ProviderName() string { return "lemonade" }
