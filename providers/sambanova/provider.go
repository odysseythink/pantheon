...
const defaultBaseURL = "https://api.sambanova.com/openai/v1"

type Provider struct {
	client *openaicompat.Client
}

// New creates a new Sambanova provider with the given API key.
...
// Option configures the Sambanova provider.
...
// Models returns a list of available models from the Sambanova API.
...
// LanguageModel creates a new Sambanova language model for the given model ID.
...
// EmbeddingModel creates a new Sambanova embedding model for the given model ID.
...
// ProviderOptions holds Sambanova-specific request options.
type ProviderOptions struct{}

// ProviderName returns the provider name for these options.
func (ProviderOptions) ProviderName() string { return "sambanova" }
