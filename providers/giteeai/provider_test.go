...
func TestProvider_LanguageModel(t *testing.T) {
	p, _ := New("sk-test")
	model, err := p.LanguageModel(context.Background(), "giteeai-medium")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if model.Provider() != "giteeai" {
		t.Errorf("unexpected provider: %s", model.Provider())
	}
	if model.Model() != "giteeai-medium" {
...
func TestProvider_EmbeddingModel(t *testing.T) {
	p, _ := New("sk-test")
	prov := p.(*Provider)
	model, err := prov.EmbeddingModel(context.Background(), "giteeai-embed")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	em := model.(*EmbeddingModel)
	if em.Provider() != "giteeai" {
		t.Errorf("unexpected provider: %s", em.Provider())
	}
	if em.Model() != "giteeai-embed" {
