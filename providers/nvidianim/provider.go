...
// New creates a new Nvidianim provider with the given API key.
func New(apiKey string, opts ...Option) (core.Provider, error) {
	...
	if p.client.BaseURL == "" {
		return nil, fmt.Errorf("nvidianim: base URL is required, use WithBaseURL")
	}
	return p, nil
}

// Option configures the Nvidianim provider.
...
// Models returns a list of available models from the Nvidianim API.
...
// LanguageModel creates a new Nvidianim language model for the given model ID.
...
// EmbeddingModel creates a new Nvidianim embedding model for the given model ID.
...
// ProviderOptions holds Nvidianim-specific request options.
type ProviderOptions struct{}

// ProviderName returns the provider name for these options.
func (ProviderOptions) ProviderName() string { return "nvidianim" }
