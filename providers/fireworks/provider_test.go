...
func TestProvider_LanguageModel(t *testing.T) {
	p, _ := New("sk-test")
	model, err := p.LanguageModel(context.Background(), "fireworks-model")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if model.Model() != "fireworks-model" {
...
func TestProvider_EmbeddingModel(t *testing.T) {
	p, _ := New("sk-test")
	prov := p.(*Provider)
	model, err := prov.EmbeddingModel(context.Background(), "fireworks-embed")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	em := model.(*EmbeddingModel)
	if em.Provider() != "fireworks" {
		t.Errorf("unexpected provider: %s", em.Provider())
	}
	if em.Model() != "fireworks-embed" {
...
func TestProviderOptions_ProviderName(t *testing.T) {
	...
	if opts.ProviderName() != "fireworks" {
