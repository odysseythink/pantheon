...
// New creates a new Privatemode provider with the given API key.
func New(apiKey string, opts ...Option) (core.Provider, error) {
	...
	if p.client.BaseURL == "" {
		return nil, fmt.Errorf("privatemode: base URL is required, use WithBaseURL")
	}
	return p, nil
}

// Option configures the Privatemode provider.
...
// Models returns a list of available models from the Privatemode API.
...
// LanguageModel creates a new Privatemode language model for the given model ID.
...
// EmbeddingModel creates a new Privatemode embedding model for the given model ID.
...
// ProviderOptions holds Privatemode-specific request options.
type ProviderOptions struct{}

// ProviderName returns the provider name for these options.
func (ProviderOptions) ProviderName() string { return "privatemode" }
