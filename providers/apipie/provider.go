...
// New creates a new Apipie provider with the given API key.
func New(apiKey string, opts ...Option) (core.Provider, error) {
	...
	if p.client.BaseURL == "" {
		return nil, fmt.Errorf("apipie: base URL is required, use WithBaseURL")
	}
	return p, nil
}

// Option configures the Apipie provider.
...
// Models returns a list of available models from the Apipie API.
...
// LanguageModel creates a new Apipie language model for the given model ID.
...
// EmbeddingModel creates a new Apipie embedding model for the given model ID.
...
// ProviderOptions holds Apipie-specific request options.
type ProviderOptions struct{}

// ProviderName returns the provider name for these options.
func (ProviderOptions) ProviderName() string { return "apipie" }
