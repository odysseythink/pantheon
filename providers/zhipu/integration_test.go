...
func TestIntegration_GenerateWithOptions(t *testing.T) {
	...
	resp, err := lm.Generate(context.Background(), &core.Request{
		...
		Messages: []core.Message{
			{Role: core.RoleUser, Content: []core.ContentPart{core.TextPart{Text: "Say hi."}}},
		},
		Temperature: &temp,
		MaxTokens:   &maxTokens,
	})
...
func TestIntegration_GenerateObjectToolMode(t *testing.T) {
	...
	resp, err := lm.GenerateObject(context.Background(), &core.ObjectRequest{
		Messages: []core.Message{
			{Role: core.RoleUser, Content: []core.ContentPart{core.TextPart{Text: "Generate a JSON object with a color field."}}},
		},
		Schema: &core.Schema{
			Type: "object",
			Properties: map[string]*core.Schema{
