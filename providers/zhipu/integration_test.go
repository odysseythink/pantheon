...
func integrationTestConfig(t *testing.T) (apiKey, model string) {
	...
	if apiKey == "" || model == "" {
		t.Skip("Skipping integration test: set ZHIPU_API_KEY and ZHIPU_MODEL to run")
	}
	return apiKey, model
}
...
func TestIntegration_GenerateObject(t *testing.T) {
	...
	if resp.Object["greeting"] == nil || resp.Object["greeting"] == "" {
		t.Error("expected non-empty greeting field")
	}
	t.Logf("Generated object: %+v", resp.Object)
}

func TestIntegration_ProviderInfo(t *testing.T) {
	apiKey, model := integrationTestConfig(t)
	lm := newIntegrationModel(t, apiKey, model)

	if lm.Provider() != "zhipu" {
		t.Errorf("unexpected provider: %s", lm.Provider())
	}
	if lm.Model() != model {
		t.Errorf("unexpected model: %s", lm.Model())
	}
}
...
func TestIntegration_GenerateWithOptions(t *testing.T) {
	...
	resp, err := lm.Generate(context.Background(), &core.Request{
		...
		Messages: []core.Message{
			{Role: core.MESSAGE_ROLE_USER, Content: []core.ContentParter{core.TextPart{Text: "Say hi."}}},
		},
		Temperature: &temp,
		MaxTokens:   &maxTokens,
	})
...
func TestIntegration_GenerateObjectToolMode(t *testing.T) {
	apiKey, model := integrationTestConfig(t)
	lm := newIntegrationModel(t, apiKey, model)

	resp, err := lm.GenerateObject(context.Background(), &core.ObjectRequest{
		Messages: []core.Message{
			{Role: core.MESSAGE_ROLE_USER, Content: []core.ContentParter{core.TextPart{Text: "Generate a JSON object with a color field."}}},
		},
		Schema: &core.Schema{
			Type: "object",
			Properties: map[string]*core.Schema{
	...
	if resp.Object["color"] == nil || resp.Object["color"] == "" {
		t.Error("expected non-empty color field")
	}
	t.Logf("Generated object: %+v", resp.Object)
}
