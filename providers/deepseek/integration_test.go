...
func integrationTestConfig(t *testing.T) (apiKey, model string) {
	...
	if apiKey == "" || model == "" {
		t.Skip("Skipping integration test: set DEEPSEEK_API_KEY and DEEPSEEK_MODEL to run")
	}
	return apiKey, model
}
...
func TestIntegration_GenerateObjectToolMode(t *testing.T) {
	...
	resp, err := lm.GenerateObject(context.Background(), &core.ObjectRequest{
		Messages: []core.Message{
			{Role: core.MESSAGE_ROLE_USER, Content: []core.ContentParter{core.TextPart{Text: "Generate a JSON object with name and age fields."}}},
		},
		Schema: &core.Schema{
			Type: "object",
			Properties: map[string]*core.Schema{
	...
	if resp.Object["name"] == nil || resp.Object["name"] == "" {
		t.Error("expected non-empty name field")
	}
	if resp.Object["age"] == nil {
