...
// New creates a new LocalAI provider with the given API key.
func New(apiKey string, opts ...Option) (core.Provider, error) {
	...
	if p.client.BaseURL == "" {
		return nil, fmt.Errorf("localai: base URL is required, use WithBaseURL")
	}
	return p, nil
}

// Option configures the LocalAI provider.
...
// Models returns a list of available models from the LocalAI API.
...
// LanguageModel creates a new LocalAI language model for the given model ID.
...
// EmbeddingModel creates a new LocalAI embedding model for the given model ID.
...
// ProviderOptions holds LocalAI-specific request options.
type ProviderOptions struct{}

// ProviderName returns the provider name for these options.
func (ProviderOptions) ProviderName() string { return "localai" }
