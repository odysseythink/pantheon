...
// New creates a new Textgenwebui provider with the given API key.
func New(apiKey string, opts ...Option) (core.Provider, error) {
	...
	if p.client.BaseURL == "" {
		return nil, fmt.Errorf("textgenwebui: base URL is required, use WithBaseURL")
	}
	return p, nil
}

// Option configures the Textgenwebui provider.
...
// Models returns a list of available models from the Textgenwebui API.
...
// LanguageModel creates a new Textgenwebui language model for the given model ID.
...
// EmbeddingModel creates a new Textgenwebui embedding model for the given model ID.
...
// ProviderOptions holds Textgenwebui-specific request options.
type ProviderOptions struct{}

// ProviderName returns the provider name for these options.
func (ProviderOptions) ProviderName() string { return "textgenwebui" }
