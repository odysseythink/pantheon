...
func TestProvider_LanguageModel(t *testing.T) {
	p, _ := New("sk-test")
	model, err := p.LanguageModel(context.Background(), "ppio-medium")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if model.Provider() != "ppio" {
		t.Errorf("unexpected provider: %s", model.Provider())
	}
	if model.Model() != "ppio-medium" {
...
func TestProvider_EmbeddingModel(t *testing.T) {
	p, _ := New("sk-test")
	prov := p.(*Provider)
	model, err := prov.EmbeddingModel(context.Background(), "ppio-embed")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
