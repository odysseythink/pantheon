...
func integrationTestConfig(t *testing.T) (accessKeyID, secretKey, region, model string) {
	t.Helper()
	accessKeyID = os.Getenv("BEDROCK_ACCESS_KEY_ID")
	secretKey = os.Getenv("BEDROCK_SECRET_KEY")
	region = os.Getenv("BEDROCK_REGION")
	model = os.Getenv("BEDROCK_MODEL")
	if accessKeyID == "" || secretKey == "" || region == "" || model == "" {
		t.Skip("Skipping integration test: set BEDROCK_ACCESS_KEY_ID, BEDROCK_SECRET_KEY, BEDROCK_REGION, BEDROCK_MODEL to run")
	}
	return accessKeyID, secretKey, region, model
}

func newIntegrationModel(t *testing.T, accessKeyID, secretKey, region, modelID string) core.LanguageModel {
	t.Helper()
	p, err := New(accessKeyID, secretKey, region)
	if err != nil {
...
func TestIntegration_Generate(t *testing.T) {
	accessKeyID, secretKey, region, model := integrationTestConfig(t)
	lm := newIntegrationModel(t, accessKeyID, secretKey, region, model)
