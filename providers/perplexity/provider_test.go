...
func TestProvider_LanguageModel(t *testing.T) {
	p, _ := New("sk-test")
	model, err := p.LanguageModel(context.Background(), "perplexity-model")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if model.Model() != "perplexity-model" {
...
func TestProvider_EmbeddingModel(t *testing.T) {
	p, _ := New("sk-test")
	prov := p.(*Provider)
	model, err := prov.EmbeddingModel(context.Background(), "perplexity-embed")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	em := model.(*EmbeddingModel)
	if em.Provider() != "perplexity" {
		t.Errorf("unexpected provider: %s", em.Provider())
	}
	if em.Model() != "perplexity-embed" {
...
func TestProviderOptions_ProviderName(t *testing.T) {
	...
	if opts.ProviderName() != "perplexity" {
