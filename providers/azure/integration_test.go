...
func integrationTestConfig(t *testing.T) (apiKey, resourceName, deployment, model string) {
	t.Helper()
	apiKey = os.Getenv("AZURE_API_KEY")
	resourceName = os.Getenv("AZURE_RESOURCE_NAME")
	deployment = os.Getenv("AZURE_DEPLOYMENT")
	model = os.Getenv("AZURE_MODEL")
	if apiKey == "" || resourceName == "" || deployment == "" || model == "" {
		t.Skip("Skipping integration test: set AZURE_API_KEY, AZURE_RESOURCE_NAME, AZURE_DEPLOYMENT, AZURE_MODEL to run")
	}
	return apiKey, resourceName, deployment, model
}

func newIntegrationModel(t *testing.T, apiKey, resourceName, deployment, modelID string) core.LanguageModel {
	t.Helper()
	p, err := New(apiKey, resourceName, deployment)
	if err != nil {
...
func TestIntegration_Generate(t *testing.T) {
	apiKey, resourceName, deployment, model := integrationTestConfig(t)
	lm := newIntegrationModel(t, apiKey, resourceName, deployment, model)
