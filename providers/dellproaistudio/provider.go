...
// New creates a new Dellproaistudio provider with the given API key.
func New(apiKey string, opts ...Option) (core.Provider, error) {
	...
	if p.client.BaseURL == "" {
		return nil, fmt.Errorf("dellproaistudio: base URL is required, use WithBaseURL")
	}
	return p, nil
}

// Option configures the Dellproaistudio provider.
...
func (p *Provider) Name() string {
	return "dellproaistudio"
}

// Models returns a list of available models from the Dellproaistudio API.
...
// LanguageModel creates a new Dellproaistudio language model for the given model ID.
...
// EmbeddingModel creates a new Dellproaistudio embedding model for the given model ID.
...
// ProviderOptions holds Dellproaistudio-specific request options.
type ProviderOptions struct{}

// ProviderName returns the provider name for these options.
func (ProviderOptions) ProviderName() string { return "dellproaistudio" }
