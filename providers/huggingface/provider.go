...
// New creates a new Huggingface provider with the given API key.
func New(apiKey string, opts ...Option) (core.Provider, error) {
	...
	if p.client.BaseURL == "" {
		return nil, fmt.Errorf("huggingface: base URL is required, use WithBaseURL")
	}
	return p, nil
}

// Option configures the Huggingface provider.
...
// Models returns a list of available models from the Huggingface API.
...
// LanguageModel creates a new Huggingface language model for the given model ID.
...
// EmbeddingModel creates a new Huggingface embedding model for the given model ID.
...
// ProviderOptions holds Huggingface-specific request options.
type ProviderOptions struct{}

// ProviderName returns the provider name for these options.
func (ProviderOptions) ProviderName() string { return "huggingface" }
