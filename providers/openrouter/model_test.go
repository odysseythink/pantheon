...
func TestGenerate(t *testing.T) {
	...
	model, _ := p.LanguageModel(context.Background(), "meta-llama/llama-3-8b")
...
func TestGenerateObject(t *testing.T) {
	...
	resp, err := model.GenerateObject(context.Background(), &core.ObjectRequest{
		Messages: []core.Message{{Role: core.RoleUser, Content: []core.ContentPart{core.TextPart{Text: "Give me weather"}}}},
...
func TestStream(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		chunks := []openaicompat.ChatCompletionResponse{
			{Model: "meta-llama/llama-3-8b", Choices: []openaicompat.Choice{{
...
func TestGenerate_Error(t *testing.T) {
	...
	p, _ := New("test-key", WithBaseURL(server.URL))
	model, _ := p.LanguageModel(context.Background(), "openai/gpt-4")
